...
const initalize_messages = async () => {
    ...
    let msgs: any = await data.map( (msg: { name: string; message: string }) => {return <Message_Template img={anonymous3_img} name={msg.name} message={msg.message}/>})
    console.log(msgs)
    for(let i = 0; i < msgs.length; i++){
        DummyData.push(msgs[i]);
    }
    return msgs
}
...
const Chat_Window = () => {
    ...
    const [messageList, set_messageList] = useState(DummyData);
    ...
    return (
        <div className='chat-window'>
            ...
            <div className='chat-window-buttons'>
                <button className='test-btn' onClick={() => initalize_messages().then(msg => {console.log(msg)})}>Test</button>
                <button onClick={() => console.log(DummyData)}>print data</button>
                <button onClick={() => initalize_messages().then(msg => {msg.map((obj: any) => {set_messageList(messageList.concat(obj))})})}>Test2</button>
            </div>
        </div>
    );
};
