...
const Chat_Window = () => {
    ...
    const [messageList, set_messageList] = useState([<Message_Template key={"admin message"} img={portraits[0]} id={"init"} name={"Admin"} message={"Started a new server, have fun =)"} timestamp={"The beginning of time"}/>]);
    // Get Message History 
    useEffect(() => {
        Socket.on("message-history", data => {
            ...
            for(let i = 0; i < data.length; i++){
                lpl.push(<Message_Template key={data[i].id.toString()} id={data[i].id} img={portraits[data[i].img_index]} name={data[i].name} message={data[i].message} timestamp={data[i].epoch_time}/>)
            }
        ...
        Socket.on("update-messages", (data: any) => {
            //console.log(data)
            let new_message = <Message_Template key={data[0].id.toString()} id={data[0].id} img={portraits[data[0].img_index]} name={data[0].name} message={data[0].message} timestamp={data[0].epoch_time}/>
